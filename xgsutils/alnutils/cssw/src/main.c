...
static void ssw_write (const s_align* a,
	...
	if (sam == 0) {	// Blast like output
		fprintf(stdout, "target_name:%s\tquery_name:%s\toptimal_score:%d\t", ref_seq->name.s, read->name.s, a->score1);
		if (a->score2 > 0) fprintf(stdout, "suboptimal_score:%d\t", a->score2);
		if (strand == 0) fprintf(stdout, "strand:+\t");
		else fprintf(stdout, "strand:-\t");
		if (a->ref_begin1 + 1 >= 0) fprintf(stdout, "target_begin:%d\t", a->ref_begin1 + 1);
		fprintf(stdout, "target_end:%d\t", a->ref_end1 + 1);
		if (a->read_begin1 + 1 >= 0) fprintf(stdout, "query_begin:%d\t", a->read_begin1 + 1);
